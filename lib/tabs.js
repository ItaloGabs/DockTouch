...
export function buildMediaTab(ext, container) {
    ...
    const musicCol = new St.BoxLayout({ vertical: true, style_class: 'widget-panel', x_expand: true });
    const infoRow = new St.BoxLayout({ style: 'spacing: 12px;' });
    
    const albumArt = new St.Bin({ style_class: 'mpris-album-art' });
    ...
    details.add_child(new St.Label({ 
        ...
        style_class: 'mpris-title',
        style: 'text-overflow: ellipsis;'
    }));
    details.add_child(new St.Label({ text: String(activePlayer?.artist || 'Idle'), style_class: 'mpris-artist' }));
    infoRow.add_child(details);
    musicCol.add_child(infoRow);

    const playback = new St.BoxLayout({ style_class: 'playback-bar' });
    ext._mediaTimeLabel = new St.Label({ text: '0:00', style: 'font-size: 9pt;' });
    playback.add_child(ext._mediaTimeLabel);
    ...
    ext._mediaProgress = new St.Widget({ style_class: 'progress-bar', width: 0 });
    progressContainer.set_child(ext._mediaProgress);
    playback.add_child(progressContainer);
    
    ext._mediaTotalLabel = new St.Label({ text: '-:--', style: 'font-size: 9pt;' });
    playback.add_child(ext._mediaTotalLabel);
    musicCol.add_child(playback);

    const controls = new St.BoxLayout({ style_class: 'player-controls-compact', x_align: Clutter.ActorAlign.CENTER });
    
    const prevBtn = new St.Button({ style_class: 'control-btn' });
    prevBtn.set_child(new St.Icon({ icon_name: 'media-skip-backward-symbolic', icon_size: 18 }));
    prevBtn.connect('clicked', () => activePlayer?.proxy?.PreviousRemote());
    
    const isPlaying = activePlayer?.proxy?.PlaybackStatus === 'Playing';
    const playBtn = new St.Button({ style_class: 'control-btn highlight' });
    ...
    const nextBtn = new St.Button({ style_class: 'control-btn' });
    nextBtn.set_child(new St.Icon({ icon_name: 'media-skip-forward-symbolic', icon_size: 18 }));
    ...
    musicCol.add_child(controls);
    container.add_child(musicCol);

    ext._startProgressTimer();
    ext._updateMediaProgress();
...
export function buildTimeTab(ext, container) {
    const now = GLib.DateTime.new_now_local();
    const timerCol = new St.BoxLayout({ vertical: true, style_class: 'widget-panel', x_expand: true });
    timerCol.add_child(new St.Label({ text: 'Timer', style: 'font-size: 14pt; font-weight: bold;' }));
    timerCol.add_child(new St.Label({ text: 'Inactive', opacity: 150 }));
    container.add_child(timerCol);

    const clockCol = new St.BoxLayout({ vertical: true, style_class: 'widget-panel', width: 220, x_align: Clutter.ActorAlign.CENTER });
    clockCol.add_child(new St.Label({ text: now.format('%H:%M'), style: 'font-size: 20pt;' }));
    clockCol.add_child(new St.Label({ text: 'Local Time', opacity: 150 }));
    container.add_child(clockCol);
}
