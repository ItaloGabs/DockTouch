...
export class Dock {
    ...
    _updateMiniPlayerVisibility() {
        ...
        if (isPlaying || (isPaused && !this._playerVisibilityExpired) || isMic || isScreen || isCaps || timerActive || isLocked) {
            ...
            // 1. Left Side (Priority: Media > Mic > Screen > Caps > Timer)
            this._miniIconContainer.visible = true;
            let leftType = 'none';
            if (isMediaActive) {
                leftType = 'media';
            ...
            if ((this.extension._isCapsLockActive || this.extension._isMicActive) && !Main.sessionMode.isLocked) {
                this._showOSD(this.extension._isMicActive ? 'mic' : 'caps', 100);
            }
        }
    }
    ...
    _showOSD(type, value) {
        ...
        // Se for Caps Lock, Mic, Screen ou Lock e estiver desativado, ocultar se estiver mostrando e não houver outro OSD ativo
        if ((isCaps || isMic || isScreen || isLock) && numericValue === 0) {
        ...
        // Se for Caps Lock, Mic ou Screen, mostrar no mini player em vez do OSD completo
        if ((isCaps || isMic || isScreen) && !Main.sessionMode.isLocked) {
            this._osdContent.visible = false;
            this._updateMiniPlayerVisibility();
            this._container.ease({
                width: normalWidth,
        ...
        } else if (isCaps || isMic || isScreen || isLock) {
            valueText = '';
        ...
        // Visibilidade conforme o tipo (Caps Lock, Mic, Screen e Lock ocultam tudo exceto o ícone)
        const isPillOnly = isCaps || isMic || isScreen || isLock;
        const isBothCapsAndMic = this.extension._isCapsLockActive && this.extension._isMicActive && !Main.sessionMode.isLocked;

        if (isBothCapsAndMic && isPillOnly && !isScreen) {
        ...
        // Só coloca timeout se NÃO for Caps Lock, Mic ou Lock
        if (!isPillOnly) {
    ...
    _updateHeaderIndicators() {
        ...
        if (this.extension._isScreenActive) {
            ...
            stopSharingBtn.connect('clicked', () => {
                ...
                if (Main.screenshotUI && Main.screenshotUI.stopRecording) {
                    try {
                ...
                const toCheck = [indicators.screenSharing, indicators.screenRecording, indicators.remoteAccess];
                    ...
                    try {
                        if (indicator.menu) {
                            ...
                            const stopItem = items.find(i => {
                                const text = i.label?.text?.toLowerCase() || '';
                                return text.includes('stop') || text.includes('parar') || text.includes('encerrar') || text.includes('desligar');
                            });
